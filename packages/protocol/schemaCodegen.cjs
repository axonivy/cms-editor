...
var schemaUri = 'https://jenkins.ivyteam.io/job/core_json-schema/job/master/lastSuccessfulBuild/artifact/workspace/ch.ivyteam.ivy.cms.editor.lsp.schema/target/editor-ts/14.0/cms-tsgen.json';
...
function writeSrc(ts) {
  ...
  nonNullTs = nonNullTs.replace(/: null \| /g, '?: '); // keep explicitly defined 'anyOf null' fields
...
if (schemaUri.startsWith('http')) {
  loadJson(schemaUri)
    .then(schema => tsGen.compile(schema, 'CMS'))
    .then(ts => writeSrc(ts));
} else {
