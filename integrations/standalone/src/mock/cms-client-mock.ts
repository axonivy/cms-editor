import { isCmsFileDataObject, isCmsStringDataObject, removeValue } from '@axonivy/cms-editor';
import type {
  Client,
  CmsActionArgs,
  CmsAddLocalesArgs,
...
export class CmsClientMock implements Client {
  ...
  private updateValue<T extends CmsUpdateStringValueArgs | CmsUpdateFileValueArgs>(args: T) {
    const co = this.cmsData.data.find(co => co.uri === args.updateObject.uri);
    if (co && (isCmsStringDataObject(co) || isCmsFileDataObject(co))) {
      co.values = { ...co.values, [args.updateObject.languageTag]: args.updateObject.value } as typeof co.values;
    }
  }
  ...
  removeLocales(args: CmsRemoveLocalesArgs): void {
    ...
    this.cmsData = {
      ...
      data: this.cmsData.data
        .filter(co => isCmsStringDataObject(co) || isCmsFileDataObject(co))
        .map(co => this.removeLocaleValues(co, args.locales))
        .filter(co => Object.entries(co.values).length !== 0)
    };
  }

  private removeLocaleValues = (co: CmsStringDataObject | CmsFileDataObject, locales: Array<string>) => ({
    ...co,
