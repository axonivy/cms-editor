...
> [!NOTE]
> The `Launch Standalone` launch config connects to a real engine and therefore requires a running engine on port 8080 with a workspace and project called `cms-test-project`. These attributes can be changed via URL parameters. Open the test project in an Ivy VS Code workspace to set up the workspace.
